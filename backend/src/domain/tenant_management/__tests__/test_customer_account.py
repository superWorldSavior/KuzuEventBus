...
from ..customer_account import (
    CustomerAccount, CustomerAccountStatus, SubscriptionPlan,
    ApiKey, SubscriptionDetails, CustomerAccountCreated,
    CustomerAccountSuspended, CustomerAccountActivated, StorageQuotaExceeded
)
from ...shared.value_objects import (
    EntityId, StorageSize, StorageUnit, EmailAddress, TenantName,
...
class TestCustomerAccount:
    ...
    def test_cannot_suspend_deleted_account(self):
        ...
        with pytest.raises(BusinessRuleViolation, match="Cannot suspend deleted account"):
    ...
    def test_cannot_activate_with_expired_subscription(self):
        ...
        with pytest.raises(BusinessRuleViolation, match="Cannot activate account with expired subscription"):
            account.activate_account()
