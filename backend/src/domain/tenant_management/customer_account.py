...
class CustomerAccount:
    ...
    email: EmailAddress = field(
        default_factory=lambda: EmailAddress("default@example.com")
    )
    ...
    # Subscription & Limits
    subscription: SubscriptionDetails = field(
        default_factory=lambda: SubscriptionDetails(
    ...
    def suspend_account(self, reason: str) -> None:
        ...
        self._add_event(
            CustomerAccountSuspended(aggregate_id=self.id, data={"reason": reason})
        )
    ...
    def activate_account(self) -> None:
        ...
        if self.subscription.is_expired():
            raise BusinessRuleViolation(
                "Cannot activate account with expired subscription"
            )
    ...
    def can_create_database(self) -> bool:
        ...
        return (
            self.status == CustomerAccountStatus.ACTIVE
            and self.database_count < self.subscription.max_databases
            and not self.subscription.is_expired()
        )
